...
diam[  ,  { m=mean(depth)
            ...
            list("my_int_feature" = myintf, "my_int_feature_2"=myintf2) }  ,  by=cut  ]
...
```R
set.seed(1024)
test_dt <- data.frame( sort(rep(letters[1:2],6)), c(1,2,3,4,6,7), sample(100,6) )
names(test_dt) <- c("id", "day", "value")
setDT(test_dt)
